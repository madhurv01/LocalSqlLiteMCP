...
import { config, userRoot } from "@/lib/config";
...
/**
 * Resolve a user-supplied path confined to that user's private directory only.
 * A relative path resolves against the user's root; absolute paths must sit
 * inside it. Rejects any attempt to reach another user's space.
 */
export function resolveUserDbPath(userId: string, input: string): string {
  const raw = input.trim();
  if (!raw) throw new PathSafetyError("Empty database path.");
  if (/^[a-z]+:\/\//i.test(raw)) {
    throw new PathSafetyError("Remote URIs are not allowed. Use a local file path.");
  ...
  if (!underRoot(abs, root)) {
    throw new PathSafetyError("Path is outside your workspace.");
  }
  const ext = extname(abs).toLowerCase();
  if (!ALLOWED_EXT.has(ext)) {
    throw new PathSafetyError(
